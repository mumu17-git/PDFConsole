...
def make_console_fields(x, y, width, height, r, g, b, value=""):
    # 全体表示用（読み取り専用・複数行）
    display_field = make_field(
        "console_display", x, y, width, height, r, g, b, value
    )
    display_field.Ff = 4096 + 1  # multiline + readonly

    # 最終行入力用（1行分、入力可）
    line_height = 18  # 1行分の高さ（調整してください）
    input_field = make_field(
        "console_input", x, y-line_height, width, line_height, r, g, b, ""
    )
    input_field.Ff = 0  # 入力可・1行

    return [display_field, input_field]
